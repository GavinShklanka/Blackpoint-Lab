...
export default function CompareView({ events, colors, onInfo, onSelect }) {
      ...
      <div className="compare__scroll">
        <table className="cmp">
          ...
          <tbody>
            ...
            <tr><th className="cmp__rowhead">Lives lost</th>{events.map((e) => <Cell key={e.id} kpi={e.kpis.fatalities}>{e.kpis.fatalities.value}</Cell>)}</tr>
            <tr><th className="cmp__rowhead">Injuries</th>{events.map((e) => <Cell key={e.id} kpi={e.kpis.injuries}>{e.kpis.injuries.value}</Cell>)}</tr>
            <tr>
              <th className="cmp__rowhead">Emergency-resource strain<br /><small>0–100 analytical proxy</small></th>
              {events.map((e) => {
                const s = e.kpis.strainIndex
                return (
                  <td key={e.id} className={s.value === maxStrain ? 'cmp__max' : ''}>
                    <div className="cmp__cellval">{s.value}<span className="kpi__strainmax">/100</span></div>
